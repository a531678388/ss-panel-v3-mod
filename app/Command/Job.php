...
class Job
{
    ...
    public static function backup()
    {
        ...
        system('mysqldump --user='.Config::get('db_username').' --password='.Config::get('db_password').' --host='.$db_address_array[0].' '.(isset($db_address_array[1])?'-P '.$db_address_array[1]:'').' '.Config::get('db_database').' announcement auto blockip bought code config coupon disconnect_ip link login_ip payback radius_ban shop speedtest ss_invite_code ss_node ss_password_reset ticket unblockip user user_token email_verify detect_list relay paylist> /tmp/ssmodbackup/mod.sql', $ret);
        ...
        foreach ($users as $user) {
            ...
            if (Config::get('notify_limit_mode') != 'false'){
                ...
                if (Config::get('notify_limit_mode') == 'per' && $user_traffic_left / $user->transfer_enable * 100 < Config::get('notify_limit_value')){
                    $under_limit = 'true';
                    $unit_text = '%';
                } else if (Config::get('notify_limit_mode') == 'mb' && $user_traffic_left / 1024 / 1024 < Config::get('notify_limit_value')){
                    $under_limit='true';
                    $unit_text='MB';
                }
