...
class Job
{
    public static function DailyJob()
    {
        ...
        Job::auto_reset_day();
        Job::reset_speedlimit();
        Job::class_expire_notice();
        echo __FUNCTION__." complete\n";
    }
    ...
    public static function CheckJob()
    {
        ...
        Job::clean_check_log();
        Job::check_alive_ip();
        Job::check_disconnect();
        Job::bought_auto_renew();
        Job::check_sspanel_update();
        Job::check_node_offline();
        Job::check_login_location();
        if (Config::get('enable_radius')=='true') {
            Job::radius_ban();
            Job::radius_unban();
        }
        Job::user_expire();
        Job::user_class_expire();
        #Job::auto_clean_expire();
        #Job::auto_clean_uncheck();
        #Job::auto_clean_unused();
        Job::auto_decrease_speedlimit();
        echo __FUNCTION__." complete\n";
    }

    public static function clean_daily_log()
    {
        echo __FUNCTION__." start......";
    ...
    public static function reset_node_bandwidth()
    {
        echo __FUNCTION__." start......";
        $nodes = Node::all();
    ...
    public static function update_qqwry()
    {
        echo __FUNCTION__." start......";
    ...
    public static function reset_speedlimit()
    {
        echo __FUNCTION__." start......";
        $shops = Shop::all();
        $shoparray = array();
        foreach ($shops as $shop) {
            $shoparray[$shop->id] = $shop;
        }
        $i = 0;
        $message = '';
        $users = User::where('class', '>', 0)->get();
        foreach ($users as $user) {
            if (isset($shoparray[$user->plan])) {
                $shop = $shoparray[$user->plan];
                $maxspeed = $shop->node_speedlimit();
                if (!empty($maxspeed)) {
                    if ($user->node_speedlimit != $maxspeed) {
                        $user->node_speedlimit = $maxspeed;
                        $user->save();
                        $message .= "用户ID".$user->id."plan".$user->plan."昨日流量".Tools::flowAutoShow($user->last_day_t)."恢复限速".$user->node_speedlimit."Mbps\n";
                        $i += 1;
                        if ($i == 100) {
    ...
    public static function class_expire_notice()
    {
        //过期前三天邮件提醒
        echo __FUNCTION__." start......";
        $users = User::where('class', '>', 0)->get();
        $notice_date = strtotime(date('Y-m-d H:i:s',strtotime('+ 3days')));
        $message = '';
        $i = 0;
        foreach ($users as $user) {
            $class_expire = strtotime($user->class_expire);
            if ($class_expire < $notice_date) {
                $subject = Config::get('appName')."-会员到期提醒";
                $to = $user->email;
                $text = "您好，您的会员还有三天就要到期了，续费请至<a href='https://xfj.one'>小飞机官网：https://xfj.one</a>" ;
                try {
                    Mail::send($to, $subject, 'news/warn.tpl', [
                        "user" => $user,"text" => $text
                    ], [
                ...
                if ($user->plan != '0') {
                    $message .= "\n用户ID".$user->id."过期时间".$user->class_expire;
                    $i += 1;
                    if ($i == 100) {
                        Telegram::send($message);
                        $message = '';
    ...
    public static function auto_reset_day()
    {
        //自动重置流量，计算每日流量
        echo __FUNCTION__." start......";
        $users = User::all();
    ...
    public static function clean_check_log()
    {
        echo __FUNCTION__." start......";
    ...
    public static function check_alive_ip(){
        //在线人数检测
        echo __FUNCTION__." start......";
    ...
    public static function check_disconnect()
    {
        //解封
        echo __FUNCTION__." start......";
    ...
    public static function bought_auto_renew()
    {
        //auto renew
        echo __FUNCTION__." start......";
    ...
    public static function check_sspanel_update()
    {
        echo __FUNCTION__." start......";
        $adminUser = User::where("is_admin", "=", "1")->get();
    ...
    public static function check_node_offline()
    {
        echo __FUNCTION__." start......";
        //节点掉线和恢复检测
    ...
    public static function check_login_location()
    {
        //登录地检测
        echo __FUNCTION__." start......";
    ...
    public static function radius_ban()
    {
        //radius禁用账户
        echo __FUNCTION__." start......";
        $users = User::all();
    ...
    public static function radius_unban()
    {
        //radius解除禁用账户
        echo __FUNCTION__." start......";
    ...
    public static function user_expire()
    {
        //账户过期
        echo __FUNCTION__." start......";
        $users = User::all();
    ...
    public static function user_class_expire()
    {
        //账户等级过期
        echo __FUNCTION__." start......";
        $users = User::where('class', '>', 0)->get();
        foreach ($users as $user) {
            if ($user->class!=0 && (((Config::get('enable_account_expire_reset')=='true' && strtotime($user->expire_in) < time()) ? $user->transfer_enable != Tools::toGB(Config::get('enable_account_expire_reset_traffic')) : true) && ((Config::get('enable_class_expire_reset')=='true' && ($user->class!=0 && strtotime($user->class_expire)<time() && strtotime($user->class_expire) > 1420041600))? $user->transfer_enable != Tools::toGB(Config::get('enable_class_expire_reset_traffic')) : true)) && strtotime($user->class_expire)<time() && strtotime($user->class_expire) > 1420041600) {
                if (Config::get('enable_class_expire_reset')=='true') {
                    ...
                    $to = $user->email;
                    $text = "您好，系统发现您的账号等级已经过期了。续费请登陆<a href='https://xfj.one'>小飞机官网https://xfj.one</a>";
                    try {
                        Mail::send($to, $subject, 'news/warn.tpl', [
                            "user" => $user,"text" => $text
                        ], [
    ...
    public static function auto_clean_expire()
    {
        //过期自动删除账户
        echo __FUNCTION__." start......";
        $users = User::all();
    ...
    public static function auto_clean_uncheck()
    {
        //没签到自动删除账户
        echo __FUNCTION__." start......";
        $users = User::all();
    ...
    public static function auto_clean_unused()
    {
        //没使用自动删除账户
        echo __FUNCTION__." start......";
        $users = User::all();
    ...
    public static function auto_decrease_speedlimit()
    {
        //超过流量自动限速
        echo __FUNCTION__." start......";
        $shops = Shop::all();
        ...
        foreach ($users as $user) {
            if (isset($shoparray[$user->plan])) {
                $shop = $shoparray[$user->plan];
                if (isset($shop)) {
                    $trigger_exist = ($shop->trigger_transfer()!= 0) ? 1:0;
                    $today_transfer = $user->u + $user->d - $user->last_day_t;
                    $reach_trigger = ($today_transfer >= Tools::toGB($shop->trigger_transfer())) ? 1:0;
                    $is_not_limited = ($user->node_speedlimit != $shop->base_speed()) ? 1:0;
                    if ($trigger_exist && $reach_trigger && $is_not_limited) {
                        $user->node_speedlimit = $shop->base_speed();
                        $user->save();
                        $message .= "用户ID".$user->id."plan".$user->plan."今日流量".Tools::flowAutoShow($today_transfer)."限速".$user->node_speedlimit."Mbps";
                        $i += 1;
                        if ($i == 100) {
                            Telegram::send($message);
                            $message = '';
